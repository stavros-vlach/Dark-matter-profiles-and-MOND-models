...
labels = ["$i_{fit}$", "$D_{fit}$", "$Y_{disk}$", "$Y_{bul}$", "$Y_{gas}$", "$M_{200}$", "$C_{200}$", "$\gamma$"]
optim_limits = [(1, 90), (0.1, 200), (0.1, 20), (0.1, 20), (0.1, 20), (0.1, 100), (0.1, 100), (1, 100)]
...
def calc_v_nihao_halo(quant200):
    i_fit, D_fit, Y_disk, Y_bul, Y_gas, M_200, C_200, gamma = quant200
    r_200 = 0.02063 * ((M_200 / M_sun) ** (1/3)) * h70
    a, b = 0.5, 3.5
    integral1 = []
    integral2 = []
    for p in (x_arr):
        '''int1 = quad(func, 0, C_200 * p / r_200, args=(a, b, gamma))
        integral1.append(int1[0])
        
        int2 = quad(func, 0, C_200, args=(a, b, gamma))
        integral2.append(int2[0])'''
        integral1.append(1)
        integral2.append(1)

    nihao_v = (10 ** 3) * (4.3 * M_200 / (M_sun * (10 ** 12)) / (x_arr / kpc)) * ((integral1 / integral2) ** (1/2))

    return nihao_v

def lnprob_withYs(x):
        # Parametrized according to https://iopscience.iop.org/article/10.3847/1538-4357/ac93fc/pdf
        i_fit, D_fit, Y_disk, Y_bul, Y_gas, M_200, C_200, gamma = x
        if not (0.1 <= i_fit <= 90) or not (0.1 < D_fit < 200) or not (0.1 < C_200 < 100) \
                or not (0.1 <= Y_disk < 20) or not (0.1 <= Y_bul <= 20) or not (0.1 < M_200):
            return -np.inf

        Vhalo = calc_v_nihao_halo(x)
        V_rot = V * np.sin(i_set) / np.sin(i_fit)

        # Vdisk = V_d(logMdisk)
        V_th = np.sqrt(Vhalo ** 2 + (Y_disk * Vdisk ** 2 + Vgas ** 2 + Y_bul * Vbul ** 2) * D_fit / D_quot)
...
def chisq(x):
    ...
    Vhalo = calc_v_nihao_halo(x)
    V_rot = V * np.sin(i_set) / np.sin(i_fit)
    mean = np.sqrt(Vhalo ** 2 + (Y_disk * Vdisk ** 2 + Y_gas * Vgas ** 2 + Y_bul * Vbul ** 2) * D_fit / D_quot)
...
V_th = np.sqrt(Vhalo ** 2 + (Y_disk * Vdisk ** 2 + Y_bul * Vbul ** 2) * (D_fit / D_quot))
...
def plot_result(x, y, yerrs, sampler):
    ...
    V_th = np.sqrt(Vhalo ** 2 + (Y_disk * Vdisk ** 2 + Y_gas * Vgas ** 2 + Y_bul * Vbul ** 2) * (D_fit / D_quot))
    ################################################
    plt.plot(x, V_th, 'red')
