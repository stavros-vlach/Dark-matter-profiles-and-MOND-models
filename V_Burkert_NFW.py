...
if PROFILE == "Burkert":
    ...
    labels = ["$log(M_{disk})$", "$log(\\rho_{0})$", "$r_{s}$", "$Y_{disk}$"]
    means = np.array([10, 5, 5, 0.5])
    optim_limits = [(5, 15), (5, 15), (0.1, 20), (0.0, 3.0)]
    text = f'Fit of {PROFILE} profile on {GALAXY_NAME} rot curve data from SPARC'
    nwalkers = 2000
    ...
    def calc_v_NFW_halo(Y):
        logMdisk, logrho0, rs, Y_disk = Y
        rho0 = 10 ** logrho0
    ...
    def func(beta, x):
        y = np.sqrt((0.5 * G * beta[0] * (3.2 * x ** 2) * (I0 * K0 - I1 * K1) / Rd) ** 2 +
                    ((G * 4 * np.pi * beta[1] * (beta[2] ** 3) * np.exp(2 / beta[4]) * (
                                (2 / beta[4]) ** (-3 / beta[4])) * (1 / beta[4]) *
                      gammaincc(3 / beta[4], (2 / beta[4]) * ((R / beta[3]) ** beta[4])) * gamma(
                                3 / beta[4])) / x_arr) ** 2 + ynew ** 2 + ybul ** 2)
        y = np.array(y)
        return y
